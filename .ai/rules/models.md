...
## Semua math harga lewat PriceCalculator; plan punya hasOne Price 1:1
Harga tidak lagi menempel di kolom plans (price/sale_price dihapus): setiap Plan punya `hasOne Price`. Seluruh perhitungan biaya (standard/package/volume/graduated, pwyw, lead magnet gratis, setup fee, metered usage) dipusatkan di `App\Services\PriceCalculator::calculate()`; jangan pernah ulangi math di blade/Livewire/CartItem. Cart: subtotalUsd -> setupFeeUsd -> couponDiscountUsd -> totalUsd(?Coupon); diskon kupon persen dari eligible subtotal, fixed = min(fixed_value, eligible); kupon TIDAK berlaku ke setup fee. CartItem::lineTotalUsd/unitPriceUsd/setupFeeUsd memanggil PriceCalculator (deferUsageCharges: true untuk cart). Renewal memakai PriceCalculator::renewalQuantity() (aggregasi usage bila usage-based) + includeSetupFee: false.
...
Semua kolom uang (prices.unit_price/setup_fee/min_price/suggested_price, cart_items.custom_amount, orders.subtotal/discount/total, order_items.unit_price/line_total/setup_fee, coupons.fixed_value) adalah integer sen. Model memakai App\Casts\MoneyCast: baca → float dolar, tulis → sen; nilai yang meleset dari grid sen >0.01 melempar InvalidArgumentException. Atribut model selalu float dolar; jangan kalikan ulang. coupons.value tetap decimal untuk kupon persen. Saat assertDatabaseHas terhadap kolom uang, gunakan nilai sen. Tiers (prices.tiers) disimpan sebagai JSON dalam sen juga (unit_price/fixed_fee int sent), tapi dibaca langsung oleh PriceCalculator dalam dolar setelah dibagi 100.
